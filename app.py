...
```python:pages/3_退休金.py
import streamlit as st

st.set_page_config(page_title="教師退休金試算", page_icon="💰")

# 將薪級與薪額轉換為字典，作為下拉選單對應使用
SALARY_GRADES = {
...
def calculate_replacement_ratio(years):
    ...
    規則：
    - 滿15年：39%
    - 16~35年：每年增加1.5%，第35年達到69%
    - 36~40年：每年增加0.5%，最高到71.5%
    """
    if years < 15:
        return 0.0
    elif years == 15:
        ...
        return 0.39 + (years - 15) * 0.015
    elif 35 < years <= 40:
        return 0.69 + (years - 35) * 0.005
    else:
        return 0.715

def main():
    st.title("💰 教師退休金（月退俸）試算模組")
    
    st.info("💡 財務評估原則：本試算依據現行公式得出，僅提供最保守之基準參考，不計入未來法規變動或通膨等外部因素，實際金額以銓敘部核定為準。")

    st.markdown("### 1. 基本資料輸入")
    col1, col2 = st.columns(2)
    
    with col1:
        years = st.number_input("教學年資（年）", min_value=1, max_value=50, value=25, step=1)
    
    with col2:
        input_method = st.radio("最後15年平均本俸設定方式", ["從薪級表選擇 (自動帶入)", "自行手動輸入"])

    if input_method == "從薪級表選擇 (自動帶入)":
        selected_grade = st.selectbox(
            "請選擇薪級（最低從 190 起算）", 
            options=list(SALARY_GRADES.keys()), 
            index=list(SALARY_GRADES.keys()).index(600)  # 預設先停在 600
        )
...
只要照這樣存成這兩個檔案，您的左側選單就會漂漂亮亮、整齊無誤了！
